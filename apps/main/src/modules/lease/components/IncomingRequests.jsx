...
const IncomingRequests = ({ leaseRequestsData, onAction }) => {
  ...
  const options = [
    ...
    { label: "Accept Request", permissions: ["change_lease"] },
    { label: "Reject Request", permissions: ["change_lease"] },
    { label: "Negotiate Terms", permissions: ["change_lease"] },
    { label: "Request More Info", permissions: ["change_lease"] }
  ];

  const incomingData = leaseRequestsData.filter(item => item.type === 'incoming');
    ...
    return [
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-gray-900 truncate">{el?.equipmentName}</h3>
          <p className="text-sm text-gray-600 mt-1 line-clamp-2">{el?.description}</p>
        ...
        <div className="flex items-center gap-1">
          <div className="w-6 h-6 bg-gray-300 rounded-full flex items-center justify-center">
            <span className="text-xs font-medium text-gray-600">
              {el?.owner?.split(' ').map(n => n[0]).join('').slice(0, 2)}
            </span>
          </div>
      ...
      <div className="space-y-1">
        ...
        <p className="text-sm text-gray-600">Duration: {el?.requestedPeriod?.split(' - ').length === 2 ? 
          Math.ceil((new Date(el?.requestedPeriod.split(' - ')[1]) - new Date(el?.requestedPeriod.split(' - ')[0])) / (1000 * 60 * 60 * 24)) + ' days' : 
          'N/A'}</p>
      </div>,
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="relative">
            <input
              type="text"
              placeholder="Equipment name, requester name, location"
              value={searchQuery}
