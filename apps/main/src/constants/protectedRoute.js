...
const ProtectedRoute = ({ isLoggedIn }) => {
  ...
  const getAccessibleRoutes = () => {
    // Logged-out users only ever get the public route set - protected and
    // vendor routes previously leaked through here regardless of login
    // state, since isLoggedIn was only consulted for permission checks.
    if (!isLoggedIn) {
      return publicRoutes;
    }
    if (isAdmin) {
